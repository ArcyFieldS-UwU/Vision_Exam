...
namespace robot_common {
    Armor::Armor(int _id, const cv::Point & initial_pos, double _dt)
        ...
        kf.processNoiseCov.at<float>(0, 0) = Q * dt5 / 20.0f;
        kf.processNoiseCov.at<float>(0, 2) = Q * dt4 / 8.0f;
        kf.processNoiseCov.at<float>(0, 4) = Q * dt3 / 6.0f;
        kf.processNoiseCov.at<float>(2, 0) = Q * dt4 / 8.0f;
        kf.processNoiseCov.at<float>(2, 2) = Q * dt3 / 3.0f;
        kf.processNoiseCov.at<float>(2, 4) = Q * dt2 / 2.0f;
        kf.processNoiseCov.at<float>(4, 0) = Q * dt3 / 6.0f;
        kf.processNoiseCov.at<float>(4, 2) = Q * dt2 / 2.0f;
        kf.processNoiseCov.at<float>(4, 4) = Q * dt;
    ...
    void Armor::correct(const cv::Point & measurement_pos) {
        ...
        z.at<float>(0) = static_cast<float>(measurement_pos.x);
        z.at<float>(1) = static_cast<float>(measurement_pos.y);
                                 ...
                                 std::vector<int> & unmatched_arm)
    {
        ...
        for (int i = 0; i < n_pos; i ++) {
            ...
            for (int j = 0; j < n_arm; j ++) {
                ...
                double dis = get_euclidean_distance(positions[i], _armors[j].pos);
                if (dis < best_dis) {
                    best_dis = dis;
                    best_idx = j;
                }
